...
/* CONNECTING MANUALLY THROUGH ARDUINO SERIAL MONITOR:
   -setup wifi hotspot (SSID = JUSTIN, pass = password. use these credentials exactly when creating hotspot)
   -type this CSV line in the serial monitor to connect: 
        jtt2525,JUSTIN,password,broker.emqx.io,1883
    -if wifi hotspot is different, replace "JUSTIN" with your hotspot name and "password" with your hotspot password
   -launch the HTML webpage
   - in the "Server" field, enter ws://broker.emqx.io:8083/mqtt. Click connnect
   - in Arduino IDE, type comma separated string for each topic you are published to (ex: 1,2,3,4 for 4 topics currently)
*/
...
const char *pub_am_or_pm        = "/b2w/am_or_pm";
...
if (length >= sizeof(cmd)) length = sizeof(cmd) - 1;
    ...
    if (inchar != '\n') {             // if the incoming character is not a newline then process byte the buffer position
      ...
      bufpos++;                       //  doing this over and over again until the end of package marker is read.
       ...
       if (bufpos  > 0) {
        ...
        pf |= get_next_token(am_or_pm, NULL, ",");
        ...
        snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_am_or_pm);
        client.publish(topic_publish, am_or_pm, 1);
