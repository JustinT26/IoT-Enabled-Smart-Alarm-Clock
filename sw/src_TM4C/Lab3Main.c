...
/* Hardware Connection Specifications
    ...
    - PE4 is ADC input from IR sensor
...
int main(void){
  ...
  Switch_Init();
	Speaker_Init();
  ST7735_InitR(0);
  ManageTime_Init(5, 1, PM , 3); //default time on startup
  Alarm_Init();
  ADC_Init();
  EnableInterrupts();
  ...
  while(1){
    ...
    // Build input index from PC5/PB4/PE3 flags
    if(PB4_Pressed){
    ...
    if(AlarmPlaying && (Distance/10 < DISTANCE_THRESHOLD_CM)){
      Alarm_Stop();
    }
  }
}
